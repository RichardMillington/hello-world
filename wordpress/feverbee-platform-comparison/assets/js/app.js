...
Write in a direct, consultancy tone. No bullet points — flowing paragraphs. Start with the single most important difference.`;
  ...
  try{
    ...
    if(AI_PROXY_URL){
      ...
      resp=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":apiKey,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:500,messages:[{role:"user",content:prompt}]})});
    }
